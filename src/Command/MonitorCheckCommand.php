...
class MonitorCheckCommand extends Command
{
    ...
    protected function updateMonitorStatus($monitor, array $checkResult): void
    {
        ...
        // Store old status for incident management
        ...
        // Handle incident management based on status changes
        $this->handleIncidentManagement($monitor, $oldStatus, $newStatus);
    }

    /**
     * Handle incident creation and resolution based on status changes
     *
     * @param \App\Model\Entity\Monitor $monitor Monitor entity
     ...
     * @return void
     */
    protected function handleIncidentManagement($monitor, string $oldStatus, string $newStatus): void
    {
        // Monitor went DOWN - create incident if it doesn't exist
        if ($newStatus === 'down' && $oldStatus !== 'down') {
            $incident = $this->incidentService->createIncident($monitor);

            if ($incident) {
                Log::info("Incident auto-created for monitor {$monitor->id}", [
                    'monitor_id' => $monitor->id,
                    'monitor_name' => $monitor->name,
        ...
        // Monitor came back UP - resolve active incidents
        if ($newStatus === 'up' && $oldStatus !== 'up') {
            $resolvedCount = $this->incidentService->autoResolveIncidents($monitor);

            if ($resolvedCount > 0) {
                Log::info("Incidents auto-resolved for monitor {$monitor->id}", [
                    'monitor_id' => $monitor->id,
                    'monitor_name' => $monitor->name,
