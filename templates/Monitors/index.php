 ...
 * @var array $stats
 */
$this->assign('title', __('Monitors'));
?>

<style>
    .monitors-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 16px; }
    .btn-add { padding: 10px 20px; background: #22c55e; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 500; text-decoration: none; display: inline-block; }
    ...
    .filters-row { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 16px; }
    .filter-group { display: flex; flex-direction: column; }
    .filter-group label { font-size: 13px; font-weight: 600; color: #444; margin-bottom: 6px; }
    .filter-group input, .filter-group select { padding: 8px 12px; border: 1px solid #d0d0d0; border-radius: 6px; font-size: 14px; background: white; }
    .filter-buttons { display: flex; gap: 8px; }
    .btn-filter { padding: 8px 16px; background: #3b82f6; color: white; border: none; border-radius: 6px; cursor: pointer; font-size: 14px; font-weight: 500; text-decoration: none; display: inline-block; }
    .btn-filter:hover { background: #2563eb; }
    .btn-clear { padding: 8px 16px; background: white; color: #666; border: 1px solid #d0d0d0; border-radius: 6px; cursor: pointer; font-size: 14px; text-decoration: none; display: inline-block; }
    .btn-clear:hover { background: #f8f9fa; }
    .monitors-table { width: 100%; background: white; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; }
    .monitors-table table { width: 100%; border-collapse: collapse; }
    .monitors-table th { background: #f8f9fa; padding: 12px 16px; text-align: left; font-size: 13px; font-weight: 600; color: #666; text-transform: uppercase; border-bottom: 2px solid #e0e0e0; }
    .monitors-table td { padding: 12px 16px; border-bottom: 1px solid #f0f0f0; font-size: 14px; vertical-align: middle; }
    .monitors-table tr:last-child td { border-bottom: none; }
    .monitors-table tbody tr:hover { background: #f8f9fa; }
    .status-indicator { display: inline-block; width: 12px; height: 12px; border-radius: 50%; margin-right: 8px; }
    .status-up { background: #22c55e; box-shadow: 0 0 8px rgba(34, 197, 94, 0.4); }
    .status-down { background: #ef4444; box-shadow: 0 0 8px rgba(239, 68, 68, 0.4); }
    .status-unknown { background: #999; }
    .monitor-name { font-weight: 600; color: #333; }
    .monitor-description { font-size: 13px; color: #666; margin-top: 4px; }
    .monitor-target { font-family: 'Courier New', monospace; font-size: 13px; color: #666; background: #f8f9fa; padding: 4px 8px; border-radius: 4px; display: inline-block; }
    .badge { display: inline-block; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: 600; text-transform: uppercase; white-space: nowrap; }
    ...
    .btn-action-view { background: #3b82f6; color: white; } .btn-action-view:hover { background: #2563eb; }
    .btn-action-edit { background: #f59e0b; color: white; } .btn-action-edit:hover { background: #d97706; }
    .btn-action-toggle { background: #8b5cf6; color: white; } .btn-action-toggle:hover { background: #7c3aed; }
    .btn-action-danger { background: #ef4444; color: white; } .btn-action-danger:hover { background: #dc2626; }
    ...
    @media (max-width: 768px) { .monitors-header { flex-direction: column; align-items: flex-start; } .stats-grid { grid-template-columns: 1fr; } .filters-row { grid-template-columns: 1fr; } .monitors-table { overflow-x: auto; } .action-buttons { flex-direction: column; } }
</style>
    ...
    <div class="filters-row">
        <div class="filter-group">
            ...
            <?= $this->Form->control('search', [
                'label' => false,
                'placeholder' => __('Name, target or description...'),
...
<div class="monitors-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($monitor->last_check_at): ?>
                                <strong><?= h($monitor->last_check_at->i18nFormat('yyyy-MM-dd')) ?></strong><br>
                        ...
                        <td>
                            <div class="action-buttons">
                                ...
                                <?= $this->Form->postLink(
                                    $monitor->active ? __('Disable') : __('Enable'),
                                    ['action' => 'toggle', $monitor->id],
                                    [
                                        'class' => 'btn-action btn-action-toggle',
                                        'confirm' => __('Are you sure you want to {0} this monitor?', $monitor->active ? __('disable') : __('enable'))
                                    ]
                                ) ?>
                                <?= $this->Form->postLink(
                                    __('Delete'),
    ...
    <?php else: ?>
        <div class="empty-state">
            ...
            <p style="font-size: 18px; margin-bottom: 8px;"><?= __('No monitors found') ?></p>
            <p style="margin-bottom: 16px;"><?= __('Try adjusting the filters or create your first monitor.') ?></p>
            <?= $this->Html->link(
...
<?php if ($monitors->count() > 0): ?>
    ...
    <div class="pagination-info">
        <?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} of {{count}} monitors')) ?>
    </div>
<?php endif; ?>
